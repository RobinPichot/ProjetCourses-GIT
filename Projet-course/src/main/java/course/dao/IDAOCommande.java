...
//	public Optional<Commande> findByIdWithRestaurants(@Param("id") Integer id);
	...
	@Query("SELECT DISTINCT c from Commande c where c.client.id = :id")
	public List<Commande> findAllCommandeByIdClient(@Param("id") Integer id);
	
	@Query("SELECT DISTINCT c from Commande c where c.id = :id")
	public Optional<Commande> findCommandeByIdEnCour(@Param("id") Integer id);
